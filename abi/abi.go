...
// quoteToAbiBytesV5 translates the QuoteV5 back into its little-endian ABI format
func quoteToAbiBytesV5(quote *pb.QuoteV5) ([]byte, error) {
	...
	expectedSize := headerSize + quoteBodyTypeSizeV5 + quoteBodySizeFieldLengthV5 + quote.GetTdQuoteBodyDescriptor().GetTdQuoteBodySize() + signedDataSizeFieldLengthV5 + quote.GetSignedDataSize() + int32(len(quote.GetExtraBytes()))
	if len(data) != int(expectedSize) {
		return nil, fmt.Errorf("quoteV5 size is %d bytes. Expected %d bytes", len(data), expectedSize)
	}
	return data, nil
}
...
// TdQuoteBodyV5ToAbiBytes translates the TDQuoteBodyV5 back into its little-endian ABI format
